...
public class ModFoodComponents {
    ...
    //自制食物、作物
    public static final FoodComponent A_BAG_OF_SUGAR = new FoodComponent.Builder().hunger(1).saturationModifier(0.3f).build();//一袋糖
    public static final FoodComponent SUGAR_GROUP = new FoodComponent.Builder().hunger(2).saturationModifier(0.5f)
            .statusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 200, 1), 1.0f).build();
    public static final FoodComponent SUGAR_CUBE = new FoodComponent.Builder().hunger(4).saturationModifier(0.6f)
            .statusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 400, 2), 1.0f).build();
    ...
    public static final FoodComponent SALTED_EGG_YOLK = new FoodComponent.Builder().hunger(1).saturationModifier(0.1f)
            .statusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 100), 1.0f).build();//咸蛋黄
    public static final FoodComponent ZANG_JIN_BO = new FoodComponent.Builder().hunger(6).saturationModifier(0.8f) //藏金波
            ...
            .statusEffect(new StatusEffectInstance(StatusEffects.HEALTH_BOOST, 2500, 4), 1.0f)
            .build();
